import React from 'react'

const VideoTitle = ({title, overview}) => {
  return (
    <div className='px-6 pt-[500px] absolute text-white w-screen aspect-video bg-gradient-to-r from-black'>
      <h1 className='font-bold text-6xl' >{title}</h1>
      <p className='w-[700px] py-10'>{overview}</p>
      <div className='flex'>
        <button className='bg-white flex pe-11 ps-6 py-3 border font-bold text-black border-black p-3 rounded-2xl hover:bg-opacity-70'> 
        <svg className='px-2' xmlns="http://www.w3.org/2000/svg" width="40"  viewBox="0 0 100 100">
        <polygon points="30,20 80,50 30,80" fill="black" />
       </svg>
        Play
        </button>
        <button className='bg-gray-700 bg-opacity-80 text-white font-bold mx-3 flex border border-black py-3 pe-6 ps-3 rounded-2xl hover:bg-opacity-60'>
        <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="40"  viewBox="0 0 50 50" className='px-2  ' >
        <path fill='white' d="M 25 2 C 12.309295 2 2 12.309295 2 25 C 2 37.690705 12.309295 48 25 48 C 37.690705 48 48 37.690705 48 25 C 48 12.309295 37.690705 2 25 2 z M 25 4 C 36.609824 4 46 13.390176 46 25 C 46 36.609824 36.609824 46 25 46 C 13.390176 46 4 36.609824 4 25 C 4 13.390176 13.390176 4 25 4 z M 25 11 A 3 3 0 0 0 22 14 A 3 3 0 0 0 25 17 A 3 3 0 0 0 28 14 A 3 3 0 0 0 25 11 z M 21 21 L 21 23 L 22 23 L 23 23 L 23 36 L 22 36 L 21 36 L 21 38 L 22 38 L 23 38 L 27 38 L 28 38 L 29 38 L 29 36 L 28 36 L 27 36 L 27 21 L 26 21 L 22 21 L 21 21 z"></path>
        </svg> More Info 
        </button>
      </div>

    </div>
  )
}

export default VideoTitle